...
table.3 <- format(table.3, digits = 0, nsmall = 0)
...
# formata a visualzaçào de casas decimais das variáveis numéricas
table.5 <- format(table.5, digits = 0, nsmall = 0)
...
## *Referências, Pacotes* {#refs-packs}
...
packs <- c("base", "kableExtra")

for(i in 1:length(packs)){
  a <- citation(packs[i])$title
  b <- citation(packs[i])$year
  c <- citation(packs[i])$note
  d <- citation(packs[i])$url
  cat(paste0(a, ". ", b, ". ", c, " ", d))
  cat('\n\n')
}
